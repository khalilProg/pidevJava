...
public class AjouterCampagneFrontController extends BaseFront {
    ...
    private void handleCreer(ActionEvent event) {
        ...
        try {
            ...
            // Strict duplicate check rule: Same types AND same date range
            for (Campagne existant : cs.recuperer()) {
                if (existant.getDateDebut() != null && existant.getDateDebut().isEqual(debut) &&
                        existant.getDateFin() != null && existant.getDateFin().isEqual(fin) &&
                        existant.getTypeSang() != null && existant.getTypeSang().equals(typeSangCombined)) {

                    showFieldError(titreError, "Une campagne avec les mêmes dates et types de sang existe déjà.");
                    return;
                }
            }

            Campagne c = new Campagne(0, titre, desc, debut, fin, LocalDateTime.now(), null, typeSangCombined);
            c.setEntiteDeCollectes(selectedEntites);

            cs.ajouter(c);
            ouvrirRapportPrediction(event, c);
    ...
    private void ouvrirRapportPrediction(ActionEvent event, Campagne campagne) {
        try {
            javafx.fxml.FXMLLoader loader = new javafx.fxml.FXMLLoader(
                    getClass().getResource("/RapportPrediction.fxml"));
            javafx.scene.Parent root = loader.load();

            RapportPredictionController ctrl = loader.getController();
            ctrl.initData(campagne);

            javafx.stage.Stage dialogStage = new javafx.stage.Stage();
            dialogStage.setTitle("Rapport de Prédiction IA");
            ...
            // After dialog closes, go back to the list
            ...
            alert.setHeaderText("Impossible d'ouvrir le rapport de prédiction");
