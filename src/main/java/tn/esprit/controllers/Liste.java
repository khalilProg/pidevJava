...
import tn.esprit.services.ServiceDon; // ⬅️ IMPORTED SERVICEDON CORRECTLY
...
public class Liste extends BaseFront {
    ...
    public void initialize() {
        ...
        try {
            ...
            ServiceDon serviceDon = new ServiceDon(); // ⬅️ FIXED INITIALIZATION TO ServiceDon

            currentClient = resolveCurrentClient();

            ObservableList<RendezVous> data = loadClientRendezVous(rdvService, qsService);
            updateSummaryCards(serviceDon); // ⬅️ PASSING CORRET VARIABLE
    ...
    private void updateSummaryCards(ServiceDon serviceDon) throws SQLException {
        if (campagnesCountText != null) {
            if (currentClient == null) {
                campagnesCountText.setText("0");
            } else {
                // ⬅️ FIXED CALCULATION: Fetches the proper count using Stream on existing `afficherAll` method
                long donCount = serviceDon.afficherAll().stream()
                        .filter(d -> d.getId_client() == currentClient.getId())
                        .count();
                campagnesCountText.setText(String.valueOf(donCount));
            }
        }
        if (nextDonText != null) {
            if (currentClient == null) {
    ...
    private void setupColumns(RendezVousService rdvService, QuestionnaireService qsService,
        ...
        campagneColumn.setCellValueFactory(cell -> {
            try {
                ...
                if (q != null) {
                    tn.esprit.entities.Campagne camp = campagneService.getCampagneById(q.getCampagneId());
                    return new SimpleStringProperty(camp != null ? camp.getTitre() : "Inconnu");
                }
        ...
        entiteColumn.setCellValueFactory(cell -> {
            try {
                tn.esprit.entities.EntiteDeCollecte e = entiteService.getEntiteById(cell.getValue().getEntite_id());
                return new SimpleStringProperty(e != null ? e.getNom() : "Inconnu");
            } catch (Exception e) {
                return new SimpleStringProperty("Erreur");
            }
        });
    ...
    private boolean matchesSearch(RendezVous rdv, String lower, QuestionnaireService qsService,
        ...
        try {
            ...
            if (q != null) {
                tn.esprit.entities.Campagne camp = campagneService.getCampagneById(q.getCampagneId());
                if (camp != null) campagneTitre = camp.getTitre();
            }
            tn.esprit.entities.EntiteDeCollecte e = entiteService.getEntiteById(rdv.getEntite_id());
            String entiteNom = e != null ? e.getNom() : "";
