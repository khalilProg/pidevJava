...
import tn.esprit.tools.MyDatabase; // ⬅️ TEAM INTEGRATION
...
public class ServiceDon {
    ...
    public ServiceDon() {
        // ⬅️ TEAM INTEGRATION: Uses the team's shared connection
    ...
    public void modifier(Don d) throws SQLException {
        String req = "UPDATE don SET id_client=?, id_entite=?, quantite=?, type_don=? WHERE id=?";
        try (PreparedStatement pst = conn.prepareStatement(req)) {
            pst.setInt(1, d.getId_client());
            pst.setInt(2, d.getId_entite());
            pst.setFloat(3, d.getQuantite());
    ...
    public List<ComboItem> getClientComboItems() throws SQLException {
        ...
        // ⬅️ TEAM INTEGRATION: Joined correctly using team's 'user_id' mapping
        String req = "SELECT c.id, u.nom, u.prenom FROM client c JOIN user u ON c.user_id = u.id";
    ...
    public List<ComboItem> getEntiteComboItems() throws SQLException {
        ...
        // We query only your 'don' table.
        // Note: Ensure the column name matches your DB (id_entite or entite_id)
        String req = "SELECT DISTINCT id_entite FROM don";

        try (Statement st = conn.createStatement();
             ResultSet rs = st.executeQuery(req)) {

            while (rs.next()) {
                int id = rs.getInt("id_entite");
                // Since we don't have a names table, we display "Entity ID: #"
            ...
            System.err.println("❌ SQL Error: Ensure the column 'id_entite' exists in table 'don'");
            e.printStackTrace();
            throw e;
        }
